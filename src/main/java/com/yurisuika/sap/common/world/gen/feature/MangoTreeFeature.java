...
public class MangoTreeFeature extends Feature<BaseTreeFeatureConfig> {
   public MangoTreeFeature(Codec<BaseTreeFeatureConfig> config) {
      super(config);
   }
   ...
   private void placeLogAt(IWorldWriter worldIn, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      this.setLogState(worldIn, pos, SapBlocks.MANGO_LOG.get().getDefaultState());
   }

   private void placeLeafAt(IWorldGenerationReader world, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      if (isAirOrLeaves(world, pos)) {
         this.setLogState(world, pos, SapBlocks.MANGO_LEAVES.get().getDefaultState().with(LeavesBlock.DISTANCE, 1));
      }
   }
   ...
   public static boolean isValidGround(IWorld world, BlockPos pos) {
      return world.getBlockState(pos).canSustainPlant(world, pos, Direction.UP, (IPlantable)SapBlocks.MANGO_SAPLING.get());
   }
