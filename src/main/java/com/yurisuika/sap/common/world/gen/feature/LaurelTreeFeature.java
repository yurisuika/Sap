...
public class LaurelTreeFeature extends Feature<BaseTreeFeatureConfig> {
   public LaurelTreeFeature(Codec<BaseTreeFeatureConfig> config) {
      super(config);
   }
   ...
   private void placeLogAt(IWorldWriter worldIn, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      this.setLogState(worldIn, pos, SapBlocks.LAUREL_LOG.get().getDefaultState());
   }

   private void placeLeafAt(IWorldGenerationReader world, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      if (isAirOrLeaves(world, pos)) {
         this.setLogState(world, pos, SapBlocks.LAUREL_LEAVES.get().getDefaultState().with(LeavesBlock.DISTANCE, 1));
      }
   }
   ...
   public static boolean isValidGround(IWorld world, BlockPos pos) {
      return world.getBlockState(pos).canSustainPlant(world, pos, Direction.UP, (IPlantable)SapBlocks.LAUREL_SAPLING.get());
   }
