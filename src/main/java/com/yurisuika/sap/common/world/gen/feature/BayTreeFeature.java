...
public class BayTreeFeature extends Feature<BaseTreeFeatureConfig> {
   public BayTreeFeature(Codec<BaseTreeFeatureConfig> config) {
      super(config);
   }
   ...
   private void placeLogAt(IWorldWriter worldIn, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      this.setLogState(worldIn, pos, SapBlocks.BAY_LOG.get().getDefaultState());
   }

   private void placeLeafAt(IWorldGenerationReader world, BlockPos pos, Random rand, BaseTreeFeatureConfig config) {
      if (isAirOrLeaves(world, pos)) {
         this.setLogState(world, pos, SapBlocks.BAY_LEAVES.get().getDefaultState().with(LeavesBlock.DISTANCE, 1));
      }
   }
   ...
   public static boolean isValidGround(IWorld world, BlockPos pos) {
      return world.getBlockState(pos).canSustainPlant(world, pos, Direction.UP, (IPlantable)SapBlocks.BAY_SAPLING.get());
   }
